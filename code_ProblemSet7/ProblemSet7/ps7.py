...
def process(url):
    ...
    """
    feed = feedparser.parse("http://rss.news.google.com/?output=rss")
...
class PhraseTrigger(Trigger):
    def __init__(self,phrase):
        self.phrase=phrase
    def evaluate(self,story):
        flag1=self.phrase in story.getTitle()
        flag2=self.phrase in story.getSubject()
        flag3=self.phrase in story.getSummary()
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    def makeTrigger(triggerMap, triggerType, params, name):
        if triggerType=='TITLE':
            ...
            for p in params:
                ans+=''.join(p)+' '
            triggerMap[name]=PhraseTrigger(ans[:-1])
            return PhraseTrigger(ans[:-1])
